...
const saveUserLocation = async () => {
  ...
  const res = await shApis.doPost(getActionUrl('saveUserLocation'), userData)
  if (res) {
...
const updateLocation = () => {
  if (user.value) {
    latitude.value = isFinite(user.value.latitude) ? user.value.latitude : null;
    longitude.value = isFinite(user.value.longitude) ? user.value.longitude : null;
...
<template>
  <CardLayout v-if="!loading && user">
    <div class="row mt-3 ">
      <div class="col-md-6" v-if-user-can="'users.clients.write'">
    ...
    <div>
      <div class="py-4 mx-auto text-center mt-2 rounded bg-opacity-10" v-if="!mapLoaded">
        ...
        <button class="btn btn-dark btn-lg mb-3 text-center" v-if-user-can="'users.clients.write'" @click="updateLocation"> Update Client Location </button>
      </div>

      <div v-if="mapLoaded" id="map" class="rounded p-3 shadow border-1 w-90 m-3" style="height: 500px"></div>
    </div>

    <div class="row align-items-end" v-if-user-can="'users.clients.write'">
      <div class="form-group col-md-9">
        <label>Location Details</label>
        <input class="form-control w-90" type="text" v-model="location" />
      </div>
      <div class="col-md-3">
        <label></label>
        <button class="btn btn-primary d-block w-100" @click="saveUserLocation">
          Save User Location
        </button>
      </div>
    </div>
